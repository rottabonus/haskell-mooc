...
import Control.Exception (SomeException, bracket, evaluate, finally, fromException, try)
...
hasElementsDuplicates expected actual =
  counterexample'
    ( "  Expected elements (in any order, duplicates allowed): " ++ show expected
        ++ "\n  Was: "
        ++ show actual
    )
...
isTodo e = case fromException e of
  Just TODO -> True
  _ -> False
...
toJSON ts = "[" ++ intercalate "," (map f ts) ++ "]"
  where
    f (i, name, outcome) = "{\"number\":" ++ show i ++ ",\"name\":" ++ show name ++ ",\"outcome\":\"" ++ show outcome ++ "\"}"
