...
instance Functor List where
  fmap _ Empty = Empty
  fmap f (LNode a r) = LNode (f a) (fmap f r)
...
count x = foldr (addCount x) 0

addCount :: (Eq a) => a -> a -> Int -> Int
addCount x a acc
  | x == a = acc + 1
  | otherwise = acc
...
inBoth a b = foldr (concatMatch b) [] a

concatMatch :: (Foldable t, Eq a) => t a -> a -> [a] -> [a]
concatMatch a b acc
  | b `elem` a = b : acc
  | otherwise = acc
...
instance Foldable List where
  foldr _ x Empty = x
  foldr f x (LNode a rest) = f a (foldr f x rest)
...
instance Foldable TwoList where
  ...
  foldr f x (TwoNode a b rest) = f a (f b (foldr f x rest))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a l r) = mappend (mappend (sumTree l) a) (sumTree r)
