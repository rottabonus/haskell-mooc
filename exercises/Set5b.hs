...
search val (Node a l r)
    | val == a = Just []
    | leftResult == Nothing = rightResult
    | otherwise = leftResult
    where 
        leftResult = case search val l of
            Just left -> Just (StepL : left)
            Nothing -> Nothing
        rightResult = case search val r of
            Just right -> Just (StepR : right)
            Nothing -> Nothing
