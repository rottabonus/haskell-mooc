...
ifM opBool opThen opElse = do
  result <- opBool
  if result then opThen else opElse
...
visit maze place = do
  ...
  unless
    ...
    $ do
      modify (place :)
      >> mapM_ (visit maze) neighbour
  where
    (Just neighbour) = lookup place maze
...
allSums :: [Int] -> [Int]
allSums xs = map sum $ subList xs

subList [] = [[]]
subList (x : xs) = subList xs ++ map (\xs -> x : xs) (subList xs)
