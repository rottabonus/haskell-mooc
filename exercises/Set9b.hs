...
prettyPrint = boardBuild (1, 1)

boardBuild :: Coord -> Int -> [Coord] -> String
boardBuild (i, j) size xs
  | i == size && j == size = sign ++ "\n"
  | j == size = sign ++ "\n" ++ boardBuild (nextRow (i, j)) size xs
  | otherwise = sign ++ boardBuild (nextCol (i, j)) size xs
  where
    sign = if (i, j) `elem` xs then "Q" else "."
...
danger _ [] = False
danger cand (queen : rest)
  | dzone cand queen = True
  | otherwise = danger cand rest
  where
    dzone c q = sameRow c q || sameCol c q || sameDiag c q || sameAntidiag c q
...
prettyPrint2 = boardBuild2 (1, 1)

boardBuild2 :: Coord -> Int -> [Coord] -> String
boardBuild2 (i, j) size xs
  | i == size && j == size = sign ++ "\n"
  | j == size = sign ++ "\n" ++ boardBuild2 (nextRow (i, j)) size xs
  | otherwise = sign ++ boardBuild2 (nextCol (i, j)) size xs
  where
    sign = if (i, j) `elem` xs then "Q" else empt
    empt = if danger (i, j) xs then "#" else "."
...
fixFirst n [] = Nothing
fixFirst n ((x, y) : queens)
  | y > n = Nothing
  | danger (x, y) queens = fixFirst n (nextCol (x, y) : queens)
  | otherwise = Just ((x, y) : queens)
...
continue (q : queens) = nextRow q : (q : queens)
