...
balance db account = do
  balanceArr <- query db balanceQuery [account] :: IO [[Int]]
  return (sum $ map sum balanceArr)
...
checkCmd textArr
  | head textArr == T.pack "balance" = Just (textArr, 2)
  | head textArr == T.pack "withdraw" = Just (textArr, 3)
  | head textArr == T.pack "deposit" = Just (textArr, 3)
  | otherwise = Nothing

checkLength :: ([T.Text], Int) -> Maybe ([T.Text], Int)
checkLength (textArr, n)
  | length textArr == n = Just (textArr, n)
  | otherwise = Nothing

checkLast :: ([T.Text], Int) -> Maybe [T.Text]
checkLast (textArr, n)
  | n == 3 = case parseInt (last textArr) of
    Just num -> Just textArr
    Nothing -> Nothing
  | otherwise = Just textArr

buildResult :: [T.Text] -> Maybe Command
buildResult text
  | cmd == T.pack "balance" = Just (Balance account)
  | cmd == T.pack "withdraw" = Just (Withdraw account amount)
  | cmd == T.pack "deposit" = Just (Deposit account amount)
  | otherwise = Nothing
  where
    cmd = head text
